...
from matrix import identity_matrix, rotation_z, scaling, translation
from ray import ray
from shape import set_transform
from sphere import glass_sphere, normalize, sphere
...
def step_impl(context):
    context.n = context.s.local_normal_at(
        context.s, point(sqrt(3) / 3, sqrt(3) / 3, sqrt(3) / 3)
    )
...
def step_impl(context):
    context.n = context.s.local_normal_at(
        context.s, point(0, sqrt(2) / 2, -sqrt(2) / 2)
    )
